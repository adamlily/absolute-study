<?php
	/*****************************************************************************
		Database Class for mysqli Server. Please do not change anything
		*****************************************************************************/
		class database {
			protected $Con;

			public function __construct($database_host,$database_port,$database_user,$database_password,$database_name) {
				$this->Con = @mysqli_connect($database_host.":".$database_port,$database_user,$database_password) or die($this->error(mysqli_errno($this->Con),__FILE__,__LINE__));
				@mysqli_select_db($database_name,$this->Con) or die($this->error(mysqli_errno($this->Con),__FILE__,__LINE__));
			}
			...
			public function query($sql,$errorFile,$errorLine) {
				$result = @mysqli_query($sql,$this->Con) or die($this->error($sql."<br />".mysqli_errno(),$errorFile,$errorLine));
				return $result;
			}
			...
			*****************************************************/
			if($Page > 1){ ?>
				...
				private function error($arg_error_msg,$arg_file=null,$arg_line=null) {
